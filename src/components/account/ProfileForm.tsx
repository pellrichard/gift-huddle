      ...
      <CardContent className='space-y-6'>
            ...
            <div className='flex items-center gap-3'>
              ...
              <label className='inline-flex items-center gap-2 text-sm'>
                <input
                  type='checkbox'
                  checked={!dobShowYear}
                  onChange={(e) => setDobShowYear(!e.target.checked)}
                />
                Hide year
              </label>
            </div>
            <p className='text-xs text-gray-500 mt-1'>
              We’ll show only day & month on your public profile when “Hide
              year” is on.
            </p>
          </div>
        </div>
        ...
        <div className='grid md:grid-cols-2 gap-6'>
          <div>
            ...
            <Input
              placeholder='e.g., LEGO, running, whisky'
              onKeyDown={(e) => {
                if (e.key === 'Enter')
                  addFromCommaInput(
          ...
          <div>
            <label className='block text-sm font-medium mb-1'>
              Preferred shops (comma-separated)
            </label>
            <Input
              placeholder='e.g., Amazon, John Lewis, Argos'
              onKeyDown={(e) => {
                if (e.key === 'Enter')
                  addFromCommaInput(
                    (e.target as HTMLInputElement).value,
