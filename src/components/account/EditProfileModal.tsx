...
function choosePreferredCurrency(opts: {
  list: Array<{ code: string; name: string | null }>
  initial: string | null | undefined
}): string {
  // 1) If we already have a value (e.g., from profile), keep it
  if (
    opts.initial &&
    typeof opts.initial === 'string' &&
    opts.initial.trim().length > 0
  ) {
  ...
  try {
    ...
    console.log('[currency-detect] timezone detection error', e)
  }
...
export function EditProfileModal({
  ...
  React.useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      notify_email:
        initial?.notify_email ?? prev.notify_email ?? Boolean(initial?.email),
      unsubscribe_all:
        initial?.unsubscribe_all ?? prev.unsubscribe_all ?? false,
      preferred_currency:
        initial?.preferred_currency ?? prev.preferred_currency ?? 'GBP',
          ...
          if (list.length > 0) {
            setCurrencies(
              list.map((i) => {
                ...
                const name =
                  typeof i === 'string' ? i : (i?.name ?? i?.code ?? '')
                const label =
                  name && code && name !== code
                    ? `${name} (${code})`
                    : name || code
            ...
            const chosen = choosePreferredCurrency({
              list,
              initial: form.preferred_currency,
            })
  ...
  if (!(form?.preferred_currency ?? '').trim())
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div className='grid gap-1'>
            ...
            <select
              ...
              onChange={(e) =>
                setField('preferred_currency', e.target.value || null)
              }
              required
  ...
  React.useEffect(() => {
    ...
    const chosen = choosePreferredCurrency({
      list: currencies.map((c: { code: string; label?: string }) => ({
        code: c.code,
        name: c.label ?? null,
      })),
      initial: form.preferred_currency,
