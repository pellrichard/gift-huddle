...
export default async function OnboardingPage({
  ...
  const editing = Array.isArray(editParam)
    ? editParam[0] === '1'
    : editParam === '1'
  ...
  const initialCats = Array.isArray(profile?.categories)
    ? profile.categories
    : []
  const initialShops = Array.isArray(profile?.preferred_shops)
    ? profile.preferred_shops
    : []
  ...
  return (
    <main className='max-w-xl mx-auto px-6 py-10'>
      ...
      <p className='text-gray-600 mt-2'>
        Choose interests and favourite shops.
      </p>
      ...
      {(err || msg || rid) && (
        <div className='mt-4 rounded-xl border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-700'>
          ...
          {msg ? (
            <div className='mt-1 text-xs opacity-80'>Details: {msg}</div>
          ) : null}
          {rid ? (
            <div className='mt-1 text-xs opacity-60'>Request ID: {rid}</div>
          ) : null}
        </div>
      )}
      ...
      >
            ...
            {CATEGORIES.map((c) => (
              ...
              >
                <input
                  ...
                  name='categories'
                  value={c}
                  defaultChecked={initialCats.includes(c)}
                />
        ...
        <div>
          <h2 className='text-lg font-medium'>
            Preferred shops (pick at least one)
          </h2>
          <div className='mt-3 grid grid-cols-2 gap-3'>
            {SHOPS.map((s) => (
              <label
              ...
              >
                <input
                  ...
                  value={s}
                  defaultChecked={initialShops.includes(s)}
                />
