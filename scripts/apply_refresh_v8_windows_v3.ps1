...
if (Test-Path $tsConfigPath) {
  ...
  # compilerOptions
  if (-not ($tsJson.PSObject.Properties.Name -contains 'compilerOptions')) {
    $tsJson | Add-Member -MemberType NoteProperty -Name compilerOptions -Value ([pscustomobject]@{}) -Force
  }
  $co = $tsJson.compilerOptions

  # baseUrl
  if (-not ($co.PSObject.Properties.Name -contains 'baseUrl')) {
    $co | Add-Member -MemberType NoteProperty -Name baseUrl -Value "." -Force
  }

  # paths
  if (-not ($co.PSObject.Properties.Name -contains 'paths')) {
    $co | Add-Member -MemberType NoteProperty -Name paths -Value ([pscustomobject]@{}) -Force
  }
  $paths = $co.paths

  if (-not ($paths.PSObject.Properties.Name -contains '@/*')) {
    $paths | Add-Member -MemberType NoteProperty -Name '@/*' -Value @('src/*') -Force
  }

  # write back
  $tsJson | ConvertTo-Json -Depth 64 | Out-File $tsConfigPath -Encoding utf8 -Force
...
if (Test-Path $pkgPath) {
  ...
  if ($pkg.PSObject.Properties.Name -contains "postcss") {
    ...
    $pkg | ConvertTo-Json -Depth 64 | Out-File $pkgPath -Encoding utf8 -Force
  }
}
