...
function normalizeFromExchangerateHost(j: any) {
  ...
  const rates =
    j?.rates && typeof j.rates === 'object'
      ? (j.rates as Record<string, number>)
      : undefined
...
function normalizeFromERAPI(j: any) {
  ...
  const date =
    typeof j?.time_last_update_utc === 'string'
...
> {
  ...
  try {
    ...
    if (j && typeof j === 'object' && 'symbols' in j && j.symbols)
      return j.symbols
  } catch {}
  try {
    const j = (await safeJson(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const base =
      typeof body?.base === 'string' && body.base.length === 3
    ...
    if (!rates || !apiBase) {
      return json(
        req,
        { ok: false, error: 'Invalid response from rates provider' },
        502
      )
    }
    ...
    const logPayload = {
      ...
      fetched_at: nowIso,
      provider,
      sample: Object.fromEntries(Object.entries(rates).slice(0, 5)),
    }
    const { error: logErr } = await sb.from('fx_rates').insert({
      base: apiBase,
      effective_date,
